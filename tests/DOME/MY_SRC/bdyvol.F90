...
CONTAINS
   ...
   SUBROUTINE bdy_vol2d( kt, kc, pua2d, pva2d, phu, phv )
      ...
      ! -------------------------------------------------------------
!jc      DO ib_bdy = 1, nb_bdy
      DO ib_bdy = 2, nb_bdy
         idx => idx_bdy(ib_bdy)
         !
         jgrd = 2                               ! correct u component
!jc         DO jb = 1, idx%nblenrim(jgrd)
         DO jb = 1, idx%nblen(jgrd)
               ii = idx%nbi(jb,jgrd)
               ij = idx%nbj(jb,jgrd)
!               IF( ii == 1 .OR. ii == jpi .OR. ij == 1 .OR. ij == jpj )  CYCLE   ! to remove ?
!jc               pua2d(ii,ij) = pua2d(ii,ij) - idx%flagu(jb,jgrd) * zubtpecor * ( tmask_i(ii,ij) * tmask_i(ii+1,ij) )
               pua2d(ii,ij) = pua2d(ii,ij) - zubtpecor * ssumask(ii,ij)
         END DO
