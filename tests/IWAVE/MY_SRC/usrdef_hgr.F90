MODULE usrdef_hgr
   ...
   !!----------------------------------------------------------------------
   !!   usr_def_hgr    : initialize the horizontal mesh for IWAVE configuration
   !!----------------------------------------------------------------------
   USE dom_oce
...
CONTAINS
   ...
   SUBROUTINE usr_def_hgr( plamt , plamu , plamv  , plamf  ,   &   ! geographic position (required)
      ...
      !!                IWAVE configuration : uniform grid spacing (rn_dx)
      ...
      !
      IF(lwp) WRITE(numout,*)
      IF(lwp) WRITE(numout,*) 'usr_def_hgr : IWAVE configuration bassin'
      ...
      DO_2D( nn_hls, nn_hls, nn_hls, nn_hls )
         !                       ! longitude   (U-point in the middle lon = 0km)
         plamt(ji,jj) = zfact * (  - 0.5 + REAL( mig(ji,0)-Ni0glo/2, wp )  )  
         plamu(ji,jj) = zfact * (          REAL( mig(ji,0)-Ni0glo/2, wp )  )
         plamv(ji,jj) = plamt(ji,jj)
         plamf(ji,jj) = plamu(ji,jj)
         !                       ! latitude    (T-point in the middle lat = 0km)
         pphit(ji,jj) = zfact * (          REAL( mjg(jj,0)-(Nj0glo-1)/2-1, wp )  )
         pphiu(ji,jj) = pphit(ji,jj)
         pphiv(ji,jj) = zfact * (    0.5 + REAL( mjg(jj,0)-(Nj0glo-1)/2-1, wp )  )
