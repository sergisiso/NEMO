...
!!$   END SUBROUTINE isf_tbl
   ...
   SUBROUTINE isf_tbl_avg( ktop, kbot, phtbl, pfrac, pe3, pvarin, pvarout )
      ...
      INTEGER,  DIMENSION(:,:)        , INTENT(in ) ::   kbot         ! bottom level of the top boundary layer
      REAL(wp), DIMENSION(:,:)        , INTENT(in ) ::   phtbl, pfrac ! fraction of bottom level to be affected by the tbl
...
!!$      INTEGER,  DIMENSION(:,:)              , INTENT(in ) ::   kbot         ! bottom level of the top boundary layer
!!$      REAL(wp), DIMENSION(:,:)              , INTENT(in ) ::   phtbl, pfrac ! fraction of bottom level to be affected by the tbl
!!$      REAL(wp), DIMENSION(:,:,:)            , INTENT(in ) ::   pe3          ! vertical scale factor
...
!!$   END SUBROUTINE isf_tbl_avg_t
   ...
   SUBROUTINE isf_tbl_lvl( pe3, ktop, phtbl, kbot, pfrac )
      ...
      REAL(wp), DIMENSION(:,:,:) , INTENT(in ) ::   pe3    ! vertical scale factor
      INTEGER,  DIMENSION(:,:)   , INTENT(in ) ::   ktop   ! top level of the top boundary layer
      REAL(wp), DIMENSION(:,:)   , INTENT(in ) ::   phtbl  ! top boundary layer thickness
      INTEGER,  DIMENSION(:,:)   , INTENT(out) ::   kbot   ! bottom level of the top boundary layer
      REAL(wp), DIMENSION(:,:)   , INTENT(out) ::   pfrac  ! fraction of bottom level in the tbl
      !!---------------------------------------------------------------------
      INTEGER ::   ji, jj
      INTEGER ::   ikt, ikb
      !!---------------------------------------------------------------------
   ...
   SUBROUTINE isf_tbl_ktop( pdep, ktop )
      ...
      !!
      !!--------------------------------------------------------------------
      REAL(wp), DIMENSION(:,:) , INTENT(inout) ::   pdep        ! top depth of the parametrisation influence
      INTEGER,  DIMENSION(:,:) , INTENT(  out) ::   ktop        ! top level affected by the ice shelf parametrisation
      !!--------------------------------------------------------------------
      INTEGER :: ji, jj
      INTEGER :: ikt
      !!--------------------------------------------------------------------
