...
#if defined key_RK3
   REAL(wp), PUBLIC, PARAMETER :: pp_stb_thres_dynubs = 1.25_wp  ! starting Courant number threshold for adaptive implicit vertical advection
   REAL(wp), PUBLIC, PARAMETER :: pp_stb_cstra_dynubs = 1.60_wp  ! stability constraint for up3 with RK3 (=1.626 in Lemarie et al 2015)
#else
   REAL(wp), PUBLIC, PARAMETER :: pp_stb_thres_dynubs = 0.15_wp  ! starting Courant number threshold for adaptive implicit vertical advection
   REAL(wp), PUBLIC, PARAMETER :: pp_stb_cstra_dynubs = 0.45_wp  ! stability constraint for up3 with MLF (=0.472 in Lemarie et al 2015)
#endif
