...
CONTAINS
   ...
   SUBROUTINE dyn_adv_up3( kt, Kbb, Kmm, puu, pvv, Krhs, pau, pav, paw )
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE dyn_adv_up3  ***
      !!
...
#define zlv_vw   zfv_f
      ...
      !
      !
      DO_2D( 0, 0, 0, 0 )                 ! we assume a uniform shear in the 1st layer : dz(u(k=1)) = dz(u(k=2)) ==> zlu = 0
         zlu_uw(ji,jj) = 0._wp
         zlv_vw(ji,jj) = 0._wp
      END_2D
      !   
      DO jk = 1, jpk-2               !  divergence of advective fluxes
         ...
         DO_2D( 0, 0, 0, 0 )                  ! add () for NP repro
            ...
            !
            zfwi = zfw(ji,jj) + zfw(ji+1,jj)  ! zfw has been multiply by 1/4 already
            ...
            zzfu_kp1 = ( zfw(ji,jj) + zfw(ji+1,jj  ) ) * ( puu(ji,jj,jk+1,Kmm) + puu(ji,jj,jk,Kmm) - gamma1 * zl_u )
            zzfv_kp1 = ( zfw(ji,jj) + zfw(ji  ,jj+1) ) * ( pvv(ji,jj,jk+1,Kmm) + pvv(ji,jj,jk,Kmm) - gamma1 * zl_v )
